...
[Zprint](https://github.com/kkinnear/zprint)'s default mode is to fully reformat
a top-level s-expression, replacing all the existing indentation and newlines
with its own. If you're not attached to your existing formatting, and you don't
mind the disruption, it works well.

I often work in a codebase where such heavy-handed reformatting would be
unwelcome. In that case, I use zprint in
[`:respect-nl`](https://github.com/kkinnear/zprint/blob/main/doc/reference.md#respect-nl)
mode, which tells it to respect the existing newlines. This typically results in
less code movement, with the caveat that the output sometimes takes more
vertical space than it needs, since zprint wasn't allowed to fold existing
newlines.

## Easy way

The easy way is to configure zprint per project. First,
`.zprint.edn` in your home directory:

```
{:search-config? true}
```

Second, `.zprint.edn` in your project root:
...
The hard part is that we're [mapping an
...
To demonstrate, here's a separate mapping that I use for [reformatting
prose](https://neovim.io/doc/user/change.html#gw) at 80 columns, regardless of
the current `textwidth`:

### `config/keymaps.lua`

```lua
-- Reformat current paragraph with 80 textwidth.
-- This leans on my.operator_register() which is defined further down.
...
Here's the code for setting up the zprint bindings.
